...
3. **Capture evidence after each step**:
   ```
   GET /api/v1/devices/{deviceId}/screenshot?path=/Users/loaner/mobai-reports/smoke-test/{bundleId}&name=flow-{N}-step-{M}
   ```
...
Create a self-contained HTML report at `/Users/loaner/mobai-reports/smoke-test/{bundleId}/index.html`.
